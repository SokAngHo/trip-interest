...
initRB = function () {
  ...
  RouteBoxer.prototype.buildGrid_ = function (vertices, range) {
    ...
    for (i = 2; this.latGrid_[i - 2] < routeBounds.getNorthEast().lat(); i++) {
      this.latGrid_.push(
        routeBoundsCenter.rhumbDestinationPoint(0, range * i).lat()
      );
    }

    // Add lines from the center out to the south
    for (i = 1; this.latGrid_[1] > routeBounds.getSouthWest().lat(); i++) {
      this.latGrid_.unshift(
        routeBoundsCenter.rhumbDestinationPoint(180, range * i).lat()
      );
    }
    ...
    this.lngGrid_.push(
      routeBoundsCenter.rhumbDestinationPoint(90, range).lng()
    );
    for (i = 2; this.lngGrid_[i - 2] < routeBounds.getNorthEast().lng(); i++) {
      this.lngGrid_.push(
        routeBoundsCenter.rhumbDestinationPoint(90, range * i).lng()
      );
    }

    // Add lines from the center out to the west
    for (i = 1; this.lngGrid_[1] > routeBounds.getSouthWest().lng(); i++) {
      this.lngGrid_.unshift(
        routeBoundsCenter.rhumbDestinationPoint(270, range * i).lng()
      );
    }
  ...
  RouteBoxer.prototype.findIntersectingCells_ = function (vertices) {
    ...
    for (var i = 1; i < vertices.length; i++) {
      ...
      var gridXY = this.getGridCoordsFromHint_(
        vertices[i],
        vertices[i - 1],
        hintXY
      );
   ...
   */
  RouteBoxer.prototype.getGridCoordsFromHint_ = function (
    latlng,
    hintlatlng,
    hint
  ) {
   ...
   */
  RouteBoxer.prototype.getGridIntersects_ = function (
    start,
    end,
    startXY,
    endXY
  ...
  RouteBoxer.prototype.getGridIntersect_ = function (start, brng, gridLineLat) {
    var d =
      this.R *
      ((gridLineLat.toRad() - start.lat().toRad()) / Math.cos(brng.toRad()));
  ...
  RouteBoxer.prototype.getCellBounds_ = function (cell) {
    return new google.maps.LatLngBounds(
      ...
      new google.maps.LatLng(
        this.latGrid_[cell[1] + 1],
        this.lngGrid_[cell[0] + 1]
      )
    );
  };
...
*/
  ...
  google.maps.LatLng.prototype.rhumbBearingTo = function (dest) {
    ...
    var dPhi = Math.log(
      Math.tan(dest.lat().toRad() / 2 + Math.PI / 4) /
        Math.tan(this.lat().toRad() / 2 + Math.PI / 4)
    );
